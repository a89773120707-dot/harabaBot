"""
diagnose_filters_v3.py — искать фильтры на /search (без mode=online).
"""
...
def main():
    ...
    try:
        ...
        log.info("=" * 60)
        log.info("ДИАГНОСТИКА ФИЛЬТРОВ v3 — /search")
        log.info("=" * 60)
        ...
        # 6. Проверить есть ли элемент с srch_fltr
        if "srch_fltr" in body:
            log.info("✅ Найдено 'srch_fltr' в HTML")
            # Найти все srch_fltr элементы
            fltr_ids = re.findall(r'id="(srch_fltr[^"]+)"', body)
            log.info(f"  srch_fltr IDs: {fltr_ids}")
        else:
            log.info("❌ 'srch_fltr' НЕ найден в HTML")

        page.screenshot(path="data/diagnose_v3.png")
        log.info(f"\n📸 data/diagnose_v3.png")

        with open("data/diagnose_v3.html", "w", encoding="utf-8") as f:
            f.write(body[:50000])
        log.info("📄 data/diagnose_v3.html")
